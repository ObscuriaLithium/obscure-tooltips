...
@Desugar
public record AccentBurstIcon(Transform transform, Optional<SoundTemplate> sound) implements TooltipIcon {

    public static AccentBurstIcon fromJson(JsonObject json) {
        ...
        final Optional<SoundTemplate> sound = json.has("sound")
                ...
                : Optional.empty();
        return new AccentBurstIcon(transform, sound);
    }
    ...
    public void applyScale(TooltipState state, GuiGraphics graphics, int x, int y) {
        ...
        final float scale = (time < 0.25f) ? (Easing.EASE_OUT_CUBIC.compute(time / 0.25f) * 1.75f) : ((time < 0.5f) ? (1.75f - 0.75f * Easing.EASE_OUT_BACK.compute((time - 0.25f) / 0.25f)) : 1.0f);
