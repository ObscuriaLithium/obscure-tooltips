...
public class EnchantmentEffect implements TooltipEffect {
    ...
    public void render(TooltipRenderer renderer, Vec2 pos, Point size) {
        ...
        buffer.vertex(matrix4f, start.x, start.y, 0).color(START_COLOR).endVertex();
        buffer.vertex(matrix4f, start.x, start.y + size.y, 0).color(START_COLOR).endVertex();
        buffer.vertex(matrix4f, start.x + width, start.y + size.y - width, 0).color(END_COLOR).endVertex();
        buffer.vertex(matrix4f, start.x + width, start.y + width, 0).color(END_COLOR).endVertex();

        buffer = renderer.bufferSource().getBuffer(RenderType.guiOverlay());
        buffer.vertex(matrix4f, start.x, start.y, 0).color(START_COLOR).endVertex();
        buffer.vertex(matrix4f, start.x + width, start.y + width, 0).color(END_COLOR).endVertex();
        buffer.vertex(matrix4f, start.x + size.x - width, start.y + width, 0).color(END_COLOR).endVertex();
        buffer.vertex(matrix4f, start.x + size.x, start.y, 0).color(START_COLOR).endVertex();

        buffer = renderer.bufferSource().getBuffer(RenderType.guiOverlay());
        buffer.vertex(matrix4f, start.x + size.x - width, start.y + width, 0).color(END_COLOR).endVertex();
        buffer.vertex(matrix4f, start.x + size.x - width, start.y + size.y - width, 0).color(END_COLOR).endVertex();
        buffer.vertex(matrix4f, start.x + size.x, start.y + size.y, 0).color(START_COLOR).endVertex();
        buffer.vertex(matrix4f, start.x + size.x, start.y, 0).color(START_COLOR).endVertex();
        ...
        if (renderer.time() - lastParticle >= 0.1f) {
            ...
            particles.add(new LineParticle(PARTICLE_START_COLOR, PARTICLE_END_COLOR, 1f, from, renderer.lerp(from, center, 0.25f)));
        }
    ...
    @Override
    public Effects.Category category() {
