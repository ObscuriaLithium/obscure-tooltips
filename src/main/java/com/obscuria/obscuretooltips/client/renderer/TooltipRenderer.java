...
import com.obscuria.obscuretooltips.ObscureTooltipsConfig;
...
public final class TooltipRenderer {
    ...
    public static boolean render(GuiGraphics context, ItemStack stack, Font font, List<ClientTooltipComponent> components, int x, int y, ClientTooltipPositioner positioner) {
        updateStyle(stack);
        if (renderStyle == null || components.isEmpty()) return false;

        final Component summaryField = TooltipUtils.getRarityName(stack);
        final Point size = calculateSize(font, components, summaryField);
        final Vector2ic rawPos = positioner.positionTooltip(context.guiWidth(), context.guiHeight(), x, y, size.x, size.y);
        final Vec2 pos = new Vec2(rawPos.x(), rawPos.y());

        context.pose().pushPose();
        context.drawManaged(() -> renderStyle.renderEffects(Effects.Order.LAYER_1_BACK, context, pos, size, tooltipSeconds));
        context.drawManaged(() -> renderStyle.renderBack(context, pos, size, tooltipSeconds, true));
        context.pose().translate(0F, 0F, 400F);
        context.drawString(Minecraft.getInstance().font, summaryField, (int) pos.x + 26, (int) pos.y + 13, 0xff505050);
        context.drawManaged(() -> renderStyle.renderEffects(Effects.Order.LAYER_2_BACK$TEXT, context, pos, size, tooltipSeconds));
        renderText(context, font, components, pos);
        renderImages(context, font, components, pos);
        context.drawManaged(() -> renderStyle.renderFront(context, stack, pos, size, tooltipSeconds));
        context.drawManaged(() -> renderStyle.renderEffects(Effects.Order.LAYER_5_FRONT, context, pos, size, tooltipSeconds));
        context.pose().popPose();

        if (stack.getItem() instanceof ArmorItem && ObscureTooltipsConfig.Client.displayArmorModels.get()) {
            final Vec2 center = renderSecondPanel(context, pos);
            equip(stack);
            renderStand(context, center.add(new Vec2(0, 26)));
        } else if (stack.getItem() instanceof TieredItem && ObscureTooltipsConfig.Client.displayToolModels.get()) {
            final Vec2 center = renderSecondPanel(context, pos);
            context.pose().pushPose();
            context.pose().translate(center.x, center.y, 500);
            context.pose().scale(2.75f, 2.75f, 2.75f);
            context.pose().mulPose(Axis.XP.rotationDegrees(-30));
            context.pose().mulPose(Axis.YP.rotationDegrees((float) (System.currentTimeMillis() / 1000.0 % 360.0) * -20f));
    ...
    private static Vec2 renderSecondPanel(GuiGraphics context, Vec2 pos) {
        context.drawManaged(() -> renderStyle.renderBack(context, pos.add(new Vec2(-40, 0)), new Point(30, 60), tooltipSeconds, false));
    ...
    private static void renderText(GuiGraphics context, Font font, List<ClientTooltipComponent> components, Vec2 pos) {
        int offset = (int) pos.y + 3;
        for (int i = 0; i < components.size(); ++i) {
            ClientTooltipComponent component = components.get(i);
            component.renderText(font, (int) pos.x + (i == 0 ? 26 : 0), offset, context.pose().last().pose(), context.bufferSource());
            offset += component.getHeight() + (i == 0 ? 13 : 0);
        }
    }

    private static void renderImages(GuiGraphics context, Font font, List<ClientTooltipComponent> components, Vec2 pos) {
        int offset = (int) pos.y + 4;
        for (int i = 0; i < components.size(); ++i) {
            ClientTooltipComponent component = components.get(i);
            component.renderImage(font, (int) pos.x, offset, context);
            offset += component.getHeight() + (i == 0 ? 13 : 0);
        }
    }

    @SuppressWarnings("deprecation")
    private static void renderStand(GuiGraphics context, Vec2 pos) {
        if (renderStand == null || Minecraft.getInstance().player == null) return;
        context.pose().pushPose();
        context.pose().translate(pos.x, pos.y, 500.0D);
        context.pose().scale(-30, -30, 30);
        context.pose().mulPose(Axis.XP.rotationDegrees(25));
        context.pose().mulPose(Axis.YP.rotationDegrees((float) (System.currentTimeMillis() / 1000.0 % 360.0) * 20f));
        ...
        RenderSystem.runAsFancy(() -> entityrenderdispatcher.render(renderStand, 0.0D, 0.0D, 0.0D, 0.0F, 1.0F,
                context.pose(), context.bufferSource(), 15728880));
        context.flush();
