...
class AccentSpinIcon(
    ...
    override fun applyScale(graphics: GuiGraphics, context: TooltipContext, x: Int, y: Int) {
        ...
        val scale = when {
            time < 0.25f -> Easing.EASE_OUT_CUBIC.compute(time / 0.25f) * 1.33f
            time < 0.5f -> 1.33f - 0.33f * Easing.EASE_OUT_CUBIC.compute((time - 0.25f) / 0.25f)
            else -> 1f
        }
    ...
    companion object {
        ...
        val CODEC: Codec<AccentSpinIcon> = RecordCodecBuilder.create { codec ->
            codec.group(
                Vec3.CODEC
                    .optionalFieldOf("offset")
                    .forGetter(AccentSpinIcon::offset),
                Codec.FLOAT
                    .optionalFieldOf("scale")
                    .forGetter(AccentSpinIcon::scale)
            ).apply(codec, ::AccentSpinIcon)
        }
    }
}